...
function loadData() {
    machines.value = storage.getMachines();
    const loadedLabels = storage.getLabels();
    const machineLabels = loadedLabels.filter(l => l.targetEntity === 'machine');
    labels.value = machineLabels;
    if (machineLabels.length > 0 && !selectedLayoutId.value) {
        selectedLayoutId.value = machineLabels[0]!.id;
    }
}
...
function handleOpenModal(machine?: Machine) {
    if (machine) {
        ...
        Object.assign(formData, machine);
    } else {
        editingMachine.value = null;
        Object.keys(formData).forEach(key => delete formData[key as keyof Machine]);
    }
...
function handleSave() {
    ...
    const machine: Machine = {
        id: editingMachine.value?.id || crypto.randomUUID(),
...
function getSelectedMachines() {
    return machines.value.filter(m => selectedMachineIds.value.includes(m.id));
}
...
async function handleExportPNG() {
    ...
    for (const machine of selected) {
        const dataUrl = await printer.renderToDataURL(layout, machine as any, { format: 'png' });
...
const hasSelection = computed(() => selectedMachineIds.value.length > 0);
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-6 md:py-8 animate-in fade-in duration-500">
    ...
    <div v-if="hasSelection" class="bg-amber-50 border border-amber-100 rounded-xl p-4 mb-6 flex flex-wrap items-center justify-between gap-4 animate-in slide-in-from-top-2">
        <div class="flex items-center gap-2 text-amber-900">
            <span class="font-semibold bg-amber-100 px-2 py-0.5 rounded text-sm">{{ selectedMachineIds.length }}</span>
    ...
    <Table
        ...
        @delete="handleDelete"
        v-model:selectedIds="selectedMachineIds"
    />
